...
func (r *Renderer_GLES) newShaderProgram(vert, frag, geo, id string, crashWhenFail bool) (s *ShaderProgram_GLES, err error) {
	...
	if fragObj, err = r.compileShader(gl.FRAGMENT_SHADER, frag); chkEX(err, "Fragment Shader compliation error on "+id+"\n", crashWhenFail) {
		return nil, err
	}
	if len(geo) > 0 {
		if geoObj, err = r.compileShader(gl.GEOMETRY_SHADER_EXT, geo); chkEX(err, "Geometry Shader compliation error on "+id+"\n", crashWhenFail) {
			return nil, err
		}
...
func (r *Renderer_GLES) Init() {
	...
	for i := 0; i < len(sys.cfg.Video.ExternalShaders); i++ {
		...
		r.postShaderSelect[i].RegisterUniforms("Texture_GLES", "TextureSize", "CurrentTime")
	}
	...
	identShader.RegisterUniforms("Texture_GLES", "TextureSize", "CurrentTime")
...
func (r *Renderer_GLES) SetDepthTest(depthTest bool) {
...
func (r *Renderer_GLES) SetDepthMask(depthMask bool) {
...
func (r *Renderer_GLES) SetFrontFace(invertFrontFace bool) {
...
func (r *Renderer_GLES) SetCullFace(doubleSided bool) {
...
func (r *Renderer_GLES) SetBlending(eq BlendEquation, src, dst BlendFunc) {
...
func (r *Renderer_GLES) prepareShadowMapPipeline(bufferIndex uint32) {
...
func (r *Renderer_GLES) prepareModelPipeline(bufferIndex uint32, env *Environment) {
...
func (r *Renderer_GLES) SetModelPipeline(eq BlendEquation, src, dst BlendFunc, depthTest, depthMask, doubleSided, invertFrontFace, useUV, useNormal, useTangent, useVertColor, useJoint0, useJoint1, useOutlineAttribute bool, numVertices, vertAttrOffset uint32) {
...
func (r *Renderer_GLES) SetMeshOulinePipeline(invertFrontFace bool, meshOutline float32) {
	r.SetFrontFace(invertFrontFace)
	r.SetDepthTest(true)
	r.SetDepthMask(true)
...
func (r *Renderer_GLES) SetModelVertexData(bufferIndex uint32, values []byte) {
	gl.BindBuffer(gl.ARRAY_BUFFER, r.modelVertexBuffer[bufferIndex])
	gl.BufferData(gl.ARRAY_BUFFER, len(values), unsafe.Pointer(&values[0]), gl.STATIC_DRAW)
}
func (r *Renderer_GLES) SetModelIndexData(bufferIndex uint32, values ...uint32) {
